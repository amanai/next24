<?php foreach($this -> blog_catalog as $item) {?>
	<div style="padding-left:<?php echo  $this -> level*20;?>px;clear:both;padding-top:3px;">
		<?php if ($item['need_subscribe'] === true){ ?>
			<div style="float:left;"><input id="ch_<?php echo $item['id'];?>" onClick='this.blur();ajax(<?php echo $item['subscribe_param'];?>);' " onChange='' type="checkbox"  value="<?php echo $item['id'];?>" <?php if ($item['subscribed']) echo 'checked'; ?>/></div>
		<?php } ?>
		...
		<?php } else { ?>
			<div style="float:left;padding-leftt:5px;padding-top:3px;"><img src="<?php echo $this -> image_url;?>icons/empty.gif" /></div>
		<?php } ?>
