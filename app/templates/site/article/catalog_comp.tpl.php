  ...
  padding: 0px;
					...
					<?foreach ($this->root as $node):?>
						<?if (count($this->child[$node['key']]) > 0):?>
							<a href="<?=Project::getRequest()->createUrl('Article', 'CompetitionCatalog', array($node['id']))?>"><img src='<?if(strpos($this->select_node['key'], $node['key']) === 0):?><?=$this -> image_url."icons/minus.gif"; ?><?else :?><?=$this -> image_url."icons/plus.gif"; ?><?endif;?>' /><b> <?=$node['name']?></b></a><div <?if(strpos($this->select_node['key'], $node['key']) === 0):?>class="open_folder" <?else :?>class="close_folder" <?endif;?>>
								<? tree($node['key'], $this->child, $this); ?>
							</div>
						<?else :?>
							<div class="item"><b>»</b> <a href="<?=Project::getRequest()->createUrl('Article', 'CompetitionCatalog', array($node['id']));?>"><?=$node['name']?></a></div>
						<?endif;?>
					<?endforeach;?>
					...
					<?function tree($key, $child, $obj) { ?>
						...
						<?foreach ($child[$key] as $subNode) :?>
							<? if (strpos($obj->select_node['key'], substr($subNode['key'], 0, -4)) === 0) : ?>
								...
								<?else :?>
									<div><b>»</b> <a href="<?=Project::getRequest()->createUrl('Article', 'CompetitionCatalog', array($subNode['id']))?>"><?=$subNode['name']?></a></div>
								<?endif;?>
							<?endif;?>
						<?endforeach;?>
					<? } ?>
